...
import static org.mate.commons.interaction.action.espresso.EspressoView.ANDROID_CONTEXTUAL_MENU_ITEMS_RESOURCE_NAME;
import static org.mate.commons.interaction.action.espresso.EspressoView.ANDROID_CONTEXTUAL_MENU_ITEMS_TEXT;
import static org.mate.commons.interaction.action.espresso.EspressoView.ANDROID_VIEW_RESOURCE_NAMES_TO_SKIP;
...
public class ActionsScreenState extends AbstractScreenState {
    ...
    /**
     * Defines the InstrumentationTest actions that are applicable on the associated screen.
     */
    ...
    public ActionsScreenState(AppScreen appScreen) {
        ...
        this.actions = null;
        this.InstrumentationTestActions = null;
    }
    ...
    public List<InstrumentationTestAction> getInstrumentationTestActions() {
        for (int i = 0; i < 3 && InstrumentationTestActions == null; i++) {
            try {
                this.InstrumentationTestActions = MATEService.getRepresentationLayer().getCurrentScreenInstrumentationTestActions();
            } catch (RemoteException | AUTCrashException e) {
                MATELog.log_warn("Unable to fetch InstrumentationTest actions after AUT has " +
        ...
        if (InstrumentationTestActions == null) {
            ...
            this.InstrumentationTestActions = new ArrayList<>();
        }

        return Collections.unmodifiableList(InstrumentationTestActions);
    }
    ...
    public List<UIAction> getActions() {
        ...
        for (Widget widget : this.widgets) {
            ...
            if ((!widget.hasChildren() || widget.isSpinnerType() || widget.isClickable()
                ...
                MATELog.log("ActionsScreenState is considering widget: " + widget.toString() +
                        " with resourceID: " + resourceID);

                String resourceName = hasResourceID ? resourceID.split("/")[1] : null;
                boolean isAndroidView = hasResourceID && (
                        resourceID.startsWith("android") ||
                                resourceID.startsWith("com.google.android") ||
                                resourceID.startsWith("com.android"));
                String text = widget.getText();
                String className = widget.getClazz();
                ...
                boolean isPartOfSnackbar = className.contains("Snackbar") ||
        ...
        for (Widget widget : widgets) {
            ...
            MATELog.log_debug("ActionsScreenState is parsing actions for Widget: " + widget);
