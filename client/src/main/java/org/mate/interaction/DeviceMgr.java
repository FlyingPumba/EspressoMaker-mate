...
public class DeviceMgr {
    ...
    /**
     * The max time to wait for an action to execute in the Representation Layer (2 min).
     */
    private final long EXECUTE_ACTION_TIMEOUT = 2 * 60 * 1000;
     ...
     */
    public ActionExecutionResult executeAction(Action action) throws AUTCrashException {
        final ActionExecutionResult[] result = {ActionExecutionResult.failure()};

        TimeoutRun.timeoutRun(() -> {
            try {
                result[0] = MATEService.getRepresentationLayer().executeAction(action);
        ...
        if (!result[0].isSuccess() && action instanceof SystemAction) {
            SystemAction systemAction = (SystemAction) action;
            // for System actions we can fall back to the MATE Server
            result[0] = ActionExecutionResult.wrapBooleanSuccess(
        ...
        // Sleep here the amount of time specified in the properties file.
        Utils.sleep(Properties.ACTIONS_THROTTLE());

        checkForCrash(result[0]);
     ...
     */
    private void checkForCrash(ActionExecutionResult actionExecutionResult) throws AUTCrashException {
        if (actionExecutionResult.isSoftCrash() || !MATEService.isRepresentationLayerAlive() || isCrashDialogPresent()) {
    ...
    public void restartApp() {
        ...
        MATELog.log("App restarted");
        Utils.sleep(Properties.DELAY_AFTER_ACTIVITY_STARTED());
    }
    ...
    /**
     * Emulates pressing the button with "OK" text.
     */
    public void pressOK() {
        try {
            MATEService.getRepresentationLayer().clickUiObjectWithText("OK");
        } catch (RemoteException | AUTCrashException e) {
            if (e.getMessage() != null) {
                MATELog.log_warn(e.getMessage());
            }
    ...
    public String getCurrentPackageName() {
        try {
            String currentPackageName = MATEService.getRepresentationLayer().getCurrentPackageName();
            if (currentPackageName != null) {
                return currentPackageName;
            }
        } catch (RemoteException | AUTCrashException | IllegalStateException e) {
            if (e.getMessage() != null) {
                MATELog.log_warn(e.getMessage());
            }
        }
    ...
    private void sendBroadcastToTracer() {
        try {
            ...
            MATELog.log_warn("Unable to send broadcast to tracer: " + e.getMessage());
        }
    }
    ...
    public void disableAnimations() {
        try {
            ...
            MATELog.log_warn("Unable to disable animations: " + e.getMessage());
        }
    }

    /**
     * Executes the given assertion on the current screen.
     * @param assertion an Espresso assertion.
     * @return true if the assertion was successful, false otherwise.
     */
    public boolean executeAssertion(EspressoAssertion assertion) {
        try {
            return MATEService.getRepresentationLayer().executeAssertion(assertion);
        } catch (RemoteException | AUTCrashException e) {
            MATELog.log_warn("An error occurred when executing EspressoAssertion on device: " + e.getMessage());
            return false;
        }
    }
}
