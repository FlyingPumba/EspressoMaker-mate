...
public class UIAbstractionLayer {
    ...
    /**
     * Returns the list of executable InstrumentationTest actions on the current screen.
     *
     * @return Returns the list of executable InstrumentationTest actions.
     */
    public List<InstrumentationTestAction> getExecutableInstrumentationTestActions() {
        return getLastScreenState().getInstrumentationTestActions();
    }
    ...
    private ActionResult executeActionUnsafe(Action action) throws AUTCrashException {
        ...
        //         if (!getExecutableInstrumentationTestActions().contains(action)) {
        //             MATELog.log_warn("Can't apply given action on current screen! Select random " +
        //                     "InstrumentationTest action.");
        //             action = Randomness.randomElement(getExecutableInstrumentationTestActions());
        //         }
        //     } else {
        ...
        ActionExecutionResult actionExecutionResult = ActionExecutionResult.failure();

        try {
            actionExecutionResult = deviceMgr.executeAction(action);
            ...
            // The app has crashed. There is probably a System dialog opened right now.
            // We close it by calling "pm clear" for the AUT's package name.
            this.deviceMgr.reinstallApp();
        ...
        if (actionExecutionResult.isSuccess()) {
        ...
        result.setClosedSoftKeyboard(actionExecutionResult.isClosedSoftKeyboard());
        ...
        while (change || retry) {
            ...
            try {
                ...
                if (!MATEService.isRepresentationLayerAlive()) {
                    // Representation Layer is disconnected: We can not analyze nor interact with
                    // current screen.
                    continue;
                }

                if (screenState.getPackageName() == null ||
                        screenState.getWidgetActions() == null) {
                    // We have incomplete information about the current screen for handling dialogs.
                    continue;
                }
    ...
    public void resetApp() {
        MATELog.log_debug("UIAbstractionLayer is resetting app");
    ...
    public void restartApp() {
        MATELog.log_debug("UIAbstractionLayer is restarting app");
    ...
    public boolean executeAssertion(EspressoAssertion assertion) {
        return deviceMgr.executeAssertion(assertion);
    }
}
