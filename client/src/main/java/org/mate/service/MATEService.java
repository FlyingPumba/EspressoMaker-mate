...
public class MATEService extends Service implements IBinder.DeathRecipient {
    ...
    /**
     * The timeout for starting the Representation Layer (3 min).
     */
    public static final int REPRESENTATION_LAYER_START_TIMEOUT = 300;
    ...
    /**
     * The class name of the Activity that is used in the Launch intent for starting the AUT.
     */
    private static String startActivityName;
    ...
    public static void ensureRepresentationLayerIsConnected() {
        ...
        try {
            representationLayerConnectionCountDown.await(REPRESENTATION_LAYER_START_TIMEOUT,
                    TimeUnit.SECONDS);
        } catch (InterruptedException e) {
            MATELog.log_debug("ensureRepresentationLayerIsConnected was interrupted");
        }
    ...
    public static void disconnectRepresentationLayer() {
        if (isRepresentationLayerAlive()) {
            try {
                ...
                MATELog.log_warn("Exception while disconnecting representation layer: " + e.getMessage());
            }
        }
    ...
    public static boolean isRepresentationLayerAlive() {
        ...
        try {
            ...
            MATELog.log_debug("Exception while pinging representation layer: " + e.getMessage());
            return false;
        }
    }
    ...
    /**
     * @return the class name of the Activity that is used in the Launch intent for starting the AUT.
     */
    public static String getStartActivityName() {
    ...
    public void onCreate() {
        log(String.format("onCreate called (%s)", getFormattedDate()));
            ...
            try {
                ...
                if (representationLayer != null) {
                    ...
                    try {
                        ...
                        MATELog.log_debug("Exception while asking representation layer to exit: "
                                + e.getMessage());
                    }
                }
    ...
    private void stopService() {
        if (representationLayer == null) {
            log(String.format("MATE Service finishing gracefully (%s)", getFormattedDate()));
            stopForeground(true);
            stopSelf();
        } else {
            log(String.format("MATE Service finishing forcefully: representation layer won't " +
                    "disconnect (%s)", getFormattedDate()));
    ...
    private boolean launchPackageName(String packageName) {
        ...
        startActivityName = intent.getComponent().getClassName();
