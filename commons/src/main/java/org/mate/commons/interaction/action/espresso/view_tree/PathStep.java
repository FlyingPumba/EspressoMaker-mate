...
public class PathStep {
    ...
    /**
     * @return The index this step should choose when moving "down" to a children.
     */
    public int getDirectionIndex() {
        if (type != PathStepType.MOVE_TO_CHILD) {
            throw new IllegalStateException("Cannot get direction index for non-child step");
        }
     ...
     */
    public @Nullable
    EspressoViewTreeNode moveFromNodeUsingDirectionIndexes(EspressoViewTreeNode node) {
        switch (type) {
            case MOVE_TO_PARENT: {
                if (node.hasParent()) {
    ...
    public List<EspressoViewTreeNode> moveFromNodeWithoutDirectionIndexes(EspressoViewTreeNode node) {
        List<EspressoViewTreeNode> result = new ArrayList<>();
