...
public class PathInTree {
     ...
     */
    public @Nullable
    EspressoViewTreeNode walkPathFromNodeUsingDirectionIndexes(EspressoViewTreeNode startingNode) {
        ...
        for (PathStep step : steps) {
            ...
            currentNode = step.moveFromNodeUsingDirectionIndexes(currentNode);
        }
     ...
     * @return
     */
    public List<EspressoViewTreeNode> walkPathFromNodeWithoutDirectionIndexes(EspressoViewTreeNode startingNode) {
        HashSet<EspressoViewTreeNode> nodesFrontier = new HashSet<>();
        nodesFrontier.add(startingNode);

        for (PathStep step : steps) {
            if (nodesFrontier.isEmpty()) {
                // we were unable to walk path completely
                break;
            }

            HashSet<EspressoViewTreeNode> newFrontier = new HashSet<>();

            for (EspressoViewTreeNode node : nodesFrontier) {
                newFrontier.addAll(step.moveFromNodeWithoutDirectionIndexes(node));
            }

            nodesFrontier = newFrontier;
        }

        return new ArrayList<>(nodesFrontier);
    }
